 ...
 * @access public
 */
class Wallstreet_Customize_Control_Tabs extends WP_Customize_Control {

	/**
	 * Wallstreet_Customize_Control_Tabs constructor.
	 *
	...
	public function render_content() {
		...
		$output .= '<div class="wallstreet-tabs-control" id="input_' . esc_attr( $this->id ) . '">';
		foreach ( $this->tabs as $value => $args ) {
			if ( ! empty( $args['controls'] ) && ( $this->tab_has_controls( $args['controls'] ) ) ) {
				$controls_attribute = json_encode( $args['controls'] );

				$output .= '<div class="wallstreet-customizer-tab">';
				...
				$output .= '</label>';
				$output .= '</div><!-- /.wallstreet-customizer-tab -->';
			}
		}
		$output .= '</div><!-- /.wallstreet-tabs-control -->';
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'wallstreet-tabs-control-script', WALLSTREET_TEMPLATE_DIR_URI . '/inc/customizer/customizer-tabs/js/script.js', array( 'jquery' ), 999, true );
		wp_enqueue_style( 'wallstreet-tabs-control-style', WALLSTREET_TEMPLATE_DIR_URI . '/inc/customizer/customizer-tabs/css/wallstreet-admin-style.css', null, 999 );

	}

	/**
	 * Enqueue the partials handler script that works synchronously with the wallstreet-tabs-control-script
	 */
	public function partials_helper_script_enqueue() {
		wp_enqueue_script( 'wallstreet-tabs-addon-script', WALLSTREET_TEMPLATE_DIR_URI . '/inc/customizer/customizer-tabs/js/customizer-addon-script.js', array( 'jquery' ), 999, true );
	}
