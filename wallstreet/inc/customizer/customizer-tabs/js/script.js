...
var wallstreet_customize_control_tabs = function ( $ ) {
	'use strict';
		...
		function () {
				...
				$( '.wallstreet-tabs-control' ).each(
					function () {
						...
						var shownCtrls = $( this ).find( '.wallstreet-customizer-tab > input:checked' ).data( 'controls' );
						showControls( customizerSection, shownCtrls );
					}
				);

				$( '.wallstreet-customizer-tab > label' ).on(
					'click', function () {
						var customizerSection = $( this ).closest( '.accordion-section' );
...
wallstreet_customize_control_tabs( jQuery );
